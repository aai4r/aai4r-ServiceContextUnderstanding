...
class resnet(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        # output0
        self.RCNN_cls_score0 = nn.Linear(2048, self.n_classes0)
        if self.class_agnostic:
            self.RCNN_bbox_pred0 = nn.Linear(2048, 4)
        else:
            self.RCNN_bbox_pred0 = nn.Linear(2048, 4 * self.n_classes0)

        # output1
        self.RCNN_cls_score1 = nn.Linear(2048, self.n_classes1)
        if self.class_agnostic:
            self.RCNN_bbox_pred1 = nn.Linear(2048, 4)
        else:
            self.RCNN_bbox_pred1 = nn.Linear(2048, 4 * self.n_classes1)
