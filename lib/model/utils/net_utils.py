...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, elem_weight=None, elem_weight_norm=False):
        N = inputs.size(0)      # num of data
        # print(N)
        C = inputs.size(1)      # num of dim (or ch)
        if self.sigmoid:
            P = F.sigmoid(inputs)
            #F.softmax(inputs)
            if targets == 0:
            ...
            #inputs = F.sigmoid(inputs)
            P = F.softmax(inputs, dim=1)               # inputs is normally 2 dims (src vs tgt)
        ...
        if elem_weight is not None:
            batch_loss = batch_loss * elem_weight

            if elem_weight_norm is True:
                batch_loss = batch_loss * N / torch.sum(elem_weight)
...
def vis_detections_ext(im, class_name, dets, box_color=(0, 0, 204), thresh=0.8, draw_text=True):
    """Visual debugging of detections."""
...
def vis_detections_ext2(im, class_name, dets, box_color=(0, 0, 204), thresh=0.8, draw_text=True):
    """Visual debugging of detections."""
    for i in range(dets.shape[0]):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
...
def vis_detections_korean_ext(im, class_name, dets, box_color=(0, 0, 204), text_color=(0, 0, 0), fontsize=20, thresh=0.8):
    """Visual debugging of detections."""
    for i in range(np.minimum(10, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
        if score > thresh:
            ...
            im_draw.text((bbox[0], bbox[1] - 25), '%s: %.2f' % (class_name, score), font=ImageFont.truetype('NanumGothic.ttf', fontsize), fill=text_color)
            im = np.array(im_pil)
...
def set_learning_rate(optimizer, lr=0.001):
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
