...
class CloudStatus(imdb):
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            try:
                cls = self._class_to_ind[obj.find(self.tag_keyword).text.strip()]
                count += 1
            except:
                print('%s has a class %s that is not in our target class list' % (filename, obj.find(self.tag_keyword).text.strip()))
                continue

        num_objs = count
        boxes = np.zeros((num_objs, 4), dtype=np.uint16)
        gt_classes = np.zeros((num_objs), dtype=np.int32)
        gt_share = np.zeros(num_objs, dtype=np.int32)
