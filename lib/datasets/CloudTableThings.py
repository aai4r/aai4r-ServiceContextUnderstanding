...
from .voc_eval_ignoreOthers import voc_eval
...
class CloudTableThings(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'CloudTableThings_' + image_set)
        self._year = 2007
        self._image_set = image_set
        self._devkit_path = cfg_d.CloudTableThings
        ...
        self._classes_original = ('__background__',  # always index 0
                         'wallet',
                         'mobile_phone',
                         'dish',
                         'chopsticks',
    ...
    def image_path_from_index(self, index):
        ...
        raise AssertionError('Path does not exist: {}'.format(os.path.join(self._data_path, 'JPEGImages', index)))
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            if obj_name in self._classes_original:
                count += 1
            else:
                print('%s has a class %s that is not in our target class list' % (filename, obj.find('name').text.strip()))
        ...
        # xmls data is 1 ~ 2049, -> 0 ~ 2048, -> should be 0 ~ 2047. check and modify
        for ix, obj in enumerate(objs):
            ...
            try:
                obj_name = obj.find('name').text.strip()
                if obj_name in self._outlery_classes:
                    cls = self._class_to_ind['outlery']
                elif obj_name in self._etc_classes:
                    cls = self._class_to_ind['etc']
                elif obj_name in self._dish_classes:
                    cls = self._class_to_ind['dish']
                else:
                    raise AssertionError('Unsupported class: %s' % obj_name)
                # print(obj_name, self._classes[cls])
            except:
                continue
